...
public class StaticMethodInvoke implements ExpressionParser {
	...
	public Object parse(ControlCode control, Object[] obj) {
		...
		try {
			if (ControlCode.FOR.equals(obj[obj.length - 2])) {
				return new ReflectionUtil(target).invoke(method, clazz, param,
						(String) obj[obj.length - 1]);
			} else
				return new ReflectionUtil(target).invoke(method, clazz, param,
						null);
		} catch (IllegalAccessException e) {
